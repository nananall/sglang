...
__global__ void load_cache_to_device_buffer_kernel(
  ...
  __shared__ int32_t s_valid_top_k;
  __shared__ int32_t s_skipped_misses;
  ...
  for (int i = tid; i < NUM_TOP_K; i += BLOCK_SIZE) {
    ...
    if (token_idx < 0) {
      // Padded top-k entries are invalid and must not participate in hash/miss logic.
  ...
  total_misses = s_valid_top_k - s_total_hits - s_newest_hit - s_skipped_misses;
